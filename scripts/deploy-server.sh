...
# S3 + SSM (no SSH). Requires the Isabelle stack to be deployed first.
set -euo pipefail
cd "$(dirname "$0")/.."

out() {
  aws cloudformation describe-stacks --region eu-north-1 --stack-name Isabelle \
...
CMD_ID=$(aws ssm send-command --region eu-north-1 \
  ...
  --comment "isabelle server deploy" \
  --parameters 'commands=[
    "set -eux",
    "aws s3 cp s3://'"$BUCKET"'/server.tgz /tmp/isabelle-server.tgz",
    "mkdir -p /opt/isabelle/app",
    "rm -rf /opt/isabelle/app.new && mkdir -p /opt/isabelle/app.new",
    "tar -xzf /tmp/isabelle-server.tgz -C /opt/isabelle/app.new",
    "bash /opt/isabelle/app.new/provision.sh '"$API_DOMAIN"' '"$WEB_DOMAIN"' '"$BACKUP_BUCKET"'",
    "rm -rf /opt/isabelle/app && mv /opt/isabelle/app.new /opt/isabelle/app",
    "chown -R isabelle:isabelle /opt/isabelle/app",
    "cd /opt/isabelle/app && sudo -u isabelle /usr/local/bin/uv sync --frozen --no-dev",
    "systemctl restart isabelle",
    "for i in $(seq 1 20); do sleep 3; curl -sf http://127.0.0.1:8001/api/health && exit 0; done; echo health check failed; exit 1"
  ]' \
